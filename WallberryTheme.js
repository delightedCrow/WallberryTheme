...
Module.register("WallberryTheme", {
  ...
  getStyles: function () {
    return [this.file("css/WallberryTheme.css"), "font-awesome.css"];
  },
  ...
  fetchPhoto: function () {
    ...
    if (this.config.queries.length > 0) {
      let query =
        this.config.queries[
          Math.floor(Math.random() * this.config.queries.length)
        ];
    ...
    req.addEventListener("load", function () {
      ...
      if (this.status === 200) {
        ...
        mod.processError(
          `The Unsplash API returned the error "${unsplashData["errors"].join(
            ", "
          )}"`
        );
      } else {
  ...
  processPhoto: function (photoData) {
    var p = {};
    let width =
      this.config.imageWidth === "auto"
        ? window.innerWidth
        : this.config.imageWidth;
    let height =
      this.config.imageHeight === "auto"
        ? window.innerHeight
    ...
    if (this.img === null) this.img = document.createElement("img");
  ...
  nunjucksEnvironment: function () {
    ...
    this._nunjucksEnvironment = new nunjucks.Environment(
      new nunjucks.WebLoader(this.file(""), { async: true, useCache: true }),
      {
        trimBlocks: true,
        lstripBlocks: true
      }
